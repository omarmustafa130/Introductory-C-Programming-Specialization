...
board_t * makeBoard(int w, int h, int numMines) {
  ...
  for (int i =0; i<h; i++)
  {
    mineBoard->board[i] = malloc(w*sizeof(*(mineBoard->board[0])));
  }
...
int countMines(board_t * b, int x, int y) {
  ...
  int w = b->width;
  int h = b->height;
  int ** mine_array = b->board;
   
  if(((x-1)>=0) && ((y)>=0))
  {
    if(IS_MINE(mine_array[y][x-1]))
    {
      count++;
    }
  }
